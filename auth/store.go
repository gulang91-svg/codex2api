...
func (s *Store) ConfirmResponsesAvailable(acc *Account) bool {
	...
	acc.mu.RLock()
	if !confirmedAt.After(acc.LastRateLimitedAt) {
		confirmedAt = acc.LastRateLimitedAt.Add(time.Nanosecond)
	}
	acc.mu.RUnlock()
	return s.ConfirmResponsesAvailableSince(acc, confirmedAt)
}
