...
| 字段 | 类型 | 默认值 | 范围 | 说明 |
|------|------|--------|------|------|
| `MaxConcurrency` | int | 2 | 1-200 | 单账号最大并发请求数 |
...
账号分组可设置 `base_concurrency_override`（`1..200`，`null` 表示不覆盖）。基础并发按“账号显式覆盖 > 所属分组中最小的有效值 > 全局 `max_concurrency`”解析；最终动态并发仍会受健康档位、用量保护和智能配速限制。
